    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "total_closed_contracts": Proposal.objects.filter(contract__signed_at__isnull=False).aggregate(
                total_closed_contracts=Sum("price_netto")
            )["total_closed_contracts"],
            "total_completed_contracts": Proposal.objects.filter(
                contract__signed_at__isnull=False, contract__protocols__signed_at__isnull=False
            ).aggregate(total_closed_contracts=Sum("price_netto"))["total_closed_contracts"],
            ...
            "top_users_proposals": User.objects.annotate(total_proposals=Count("proposal_created_by")).values(
                "username", "total_proposals"
            ),
            "top_users_closed_contracts": User.objects.filter(contract_created_by__in=cs)
            .annotate(num_contracts=Count("contract_created_by"))
            .values("username", "num_contracts"),
            "top_users_protocols": User.objects.annotate(total_protocols=Count("protocol_created_by")).values(
                "username", "total_protocols"
            ),
        }
...
def get_average_proposals_per_day(working_days):
    daily_proposals = (
        Proposal.objects.annotate(date=TruncDate("created_at"))
        .values("date")
...
def get_average_closing_time():
    ...
    proposals = Proposal.objects.annotate(c_signed_at=Subquery(contracts.values("signed_at")[:1]))
    proposals = proposals.annotate(
        days_until_signed=ExpressionWrapper(F("c_signed_at") - F("created_at"), output_field=DurationField())
    )
